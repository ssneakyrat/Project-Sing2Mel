...
class DirectFeatureEncoder(nn.Module):
    """
    Combined encoder that directly maps from linguistic features to control parameters.
    Uses a hybrid architecture with convolutional layers and self-attention without
    generating mel spectrograms as an intermediate step.
    """
    def __init__(self, 
                 output_splits,
                 ...
                 num_conv_blocks=3,
                 dropout=0.1):
        super(DirectFeatureEncoder, self).__init__()
        
        # Store output splits
        self.output_splits = output_splits
        
        # Calculate total input dimension
        self.f0_dim = 32  # Projected dimension for f0
        total_input_dim = phoneme_embed_dim + singer_embed_dim + language_embed_dim + self.f0_dim
        
        # F0 processing with MLP
        self.f0_processor = nn.Sequential(
            nn.Linear(1, 16),
            nn.ReLU(),
            nn.Linear(16, self.f0_dim),
            nn.LayerNorm(self.f0_dim)
        )
        ...
        for _ in range(num_transformer_layers):
            self.transformer_layers.append(TransformerEncoderLayer(hidden_dim, num_heads, hidden_dim * 4, dropout))
        ...
        # 4. Output projection
        total_output_dim = sum([v for k, v in output_splits.items()])
        self.output_projection = nn.Linear(hidden_dim // 2, total_output_dim)
        
    def forward(self, f0, phoneme_emb, singer_emb, language_emb):
        """
        Forward pass to directly generate control parameters from linguistic features.
        ...
        # Process f0 
        f0_processed = self.f0_processor(f0)  # [B, T, f0_dim]
        
        # Expand singer and language embeddings to match sequence length
        singer_expanded = singer_emb.unsqueeze(1).expand(-1, seq_len, -1)  # [B, T, singer_embed_dim]
        language_expanded = language_emb.unsqueeze(1).expand(-1, seq_len, -1)  # [B, T, language_embed_dim]
        
        # Concatenate all inputs
        combined_input = torch.cat([
            phoneme_emb,        # [B, T, phoneme_embed_dim]
            f0_processed,       # [B, T, f0_dim]
            singer_expanded,    # [B, T, singer_embed_dim]
            language_expanded   # [B, T, language_embed_dim]
        ], dim=-1)
        ...
        # Apply convolutional blocks for local feature extraction
        for conv_block in self.conv_blocks:
            x = conv_block(x)
        
        # Apply transformer layers for global feature extraction
        for transformer_layer in self.transformer_layers:
            x = transformer_layer(x)
        
        # Add singer and language context for conditioning the output
        context_input = torch.cat([x, singer_expanded, language_expanded], dim=-1)
        x = self.context_layers(context_input)
        
        # Project to output parameters
        output = self.output_projection(x)  # [B, T, total_output_dim]
        
        # Split into dictionary of control parameters
        controls = split_to_dict(output, self.output_splits)
